...
export function NewMembersBanner() {
    ...
    if (isLoading) {
        return (
            <div className="mb-4 h-10 overflow-hidden rounded-xl bg-gradient-to-r from-green-500/5 via-[#7C8FFF]/5 to-purple-500/5 border border-white/5 animate-pulse" />
        );
    }

    if (newMembers.length === 0) {
        return (
            <div className="mb-4 overflow-hidden rounded-xl bg-gradient-to-r from-[#7C8FFF]/10 to-[#6070EE]/10 border border-[#7C8FFF]/20">
                <div className="py-2 px-4 flex items-center gap-3">
                    <span className="text-lg">👥</span>
                    <span className="text-sm text-white/70">Be among the first! Invite friends to grow the community.</span>
                    <Link href="/invite" className="ml-auto text-xs text-[#7C8FFF] font-semibold hover:underline">Invite →</Link>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="mb-4 overflow-hidden rounded-xl bg-gradient-to-r from-green-500/10 via-[#7C8FFF]/10 to-purple-500/10 border border-green-500/20">
            <div className="py-2 px-4 flex items-center gap-3">
                    ...
                    >
                        ...
                        {[...newMembers, ...newMembers].map((member, idx) => (
                            <Link
                                ...
                                className="flex items-center gap-2 text-sm text-white hover:text-[#7C8FFF] transition-colors"
                            >
                                <span className="text-lg">👋</span>
                                <span>
                                    Welcome <span className="font-semibold text-[#7C8FFF]">{member.displayName || member.username}</span>!
                                </span>
                            </Link>
                        ))}
                    </motion.div>
