...
export function PresenceProvider({ children, userId }: PresenceProviderProps) {
    ...
    useEffect(() => {
        ...
        const mockUsers: PresenceUser[] = [
            { id: 'user-1', displayName: 'Sarah Ahmed', status: 'online', currentActivity: 'Browsing feed' },
            { id: 'user-2', displayName: 'Omar Hassan', status: 'online' },
            { id: 'user-3', displayName: 'Fatima Ali', status: 'away', lastSeen: new Date(Date.now() - 5 * 60000) },
            { id: 'user-4', displayName: 'Yusuf Khan', status: 'busy', currentActivity: 'In a call' },
            { id: 'user-5', displayName: 'Aisha Mahmoud', status: 'online' },
        ];

        const usersMap = new Map(mockUsers.map(u => [u.id, u]));
        setState(prev => ({ ...prev, onlineUsers: usersMap }));
...
export function OnlineUsersBar({ maxShow = 5 }: OnlineUsersBarProps) {
    ...
    const [mockUsers] = useState<PresenceUser[]>([
        { id: '1', displayName: 'Sarah', status: 'online' },
        { id: '2', displayName: 'Omar', status: 'online' },
        { id: '3', displayName: 'Fatima', status: 'online' },
        { id: '4', displayName: 'Yusuf', status: 'away' },
        { id: '5', displayName: 'Aisha', status: 'online' },
        { id: '6', displayName: 'Ali', status: 'online' },
    ]);
