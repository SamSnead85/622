...
export function AvatarCropper({ imageSrc, onSave, onCancel }: AvatarCropperProps) {
    ...
    return (
        ...
        >
            ...
            <motion.div
                className="relative bg-[#0A0A0F] rounded-3xl border border-[#7C8FFF]/20 p-6 max-w-sm w-full shadow-2xl shadow-[#7C8FFF]/10"
            ...
            >
                ...
                <div className="flex items-center justify-between mb-4">
                    ...
                    <button
                        onClick={handleReset}
                        className="px-3 py-1.5 text-xs rounded-lg bg-white/5 border border-white/10 text-white/60 hover:text-white hover:border-[#7C8FFF]/30 transition-colors"
                    >
                        Reset
                    </button>
                </div>
                ...
                >
                    ...
                    <div
                        className="w-full h-full rounded-full overflow-hidden cursor-move border-4 border-[#7C8FFF]/40 shadow-lg shadow-[#7C8FFF]/20"
                    ...
                    <div className="absolute inset-0 pointer-events-none">
                        <div className="absolute -top-1 -left-1 w-4 h-4 border-t-2 border-l-2 border-[#7C8FFF] rounded-tl-full" />
                        <div className="absolute -top-1 -right-1 w-4 h-4 border-t-2 border-r-2 border-[#7C8FFF] rounded-tr-full" />
                        <div className="absolute -bottom-1 -left-1 w-4 h-4 border-b-2 border-l-2 border-[#7C8FFF] rounded-bl-full" />
                        <div className="absolute -bottom-1 -right-1 w-4 h-4 border-b-2 border-r-2 border-[#7C8FFF] rounded-br-full" />
                    </div>

                    {/* Loading overlay */}
                    {!imageLoaded && (
                        <div className="absolute inset-0 flex items-center justify-center bg-black/80 rounded-full">
                            <div className="w-8 h-8 border-3 border-[#7C8FFF]/30 border-t-[#7C8FFF] rounded-full animate-spin" />
                        </div>
                    )}
                </div>
                ...
                <div className="mb-6">
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className="w-8 h-8 flex items-center justify-center rounded-lg bg-white/5 border border-white/10 text-white/60 hover:text-white hover:border-[#7C8FFF]/30 transition-colors"
                        >
                            −
                        </button>
                        <div className="flex-1 relative">
                            <input
                                ...
                                style={{
                                    background: `linear-gradient(to right, #7C8FFF 0%, #7C8FFF ${((scale - 0.05) / 3.95) * 100}%, rgba(255,255,255,0.1) ${((scale - 0.05) / 3.95) * 100}%, rgba(255,255,255,0.1) 100%)`
                                }}
                            />
                        </div>
                        <button
                ...
                <div className="flex gap-3">
                    ...
                    <button
                        ...
                        className="flex-1 py-3 rounded-xl bg-gradient-to-r from-[#7C8FFF] to-[#6070EE] text-white font-semibold hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2"
                    >
                        {isSaving ? (
                            <>
