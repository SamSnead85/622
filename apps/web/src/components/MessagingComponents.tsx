            ...
            {isRecording ? (
                <>
                    ...
                    <button
                        ...
                        className="w-10 h-10 rounded-full bg-[#00D4FF] text-black flex items-center justify-center hover:opacity-90 transition-opacity"
                    >
                        ✓
                    </button>
                </>
...
export function VoiceMessagePlayer({ src, duration, isOwn = false }: VoiceMessagePlayerProps) {
    ...
    return (
        <div className={`flex items-center gap-3 p-3 rounded-2xl ${isOwn ? 'bg-[#00D4FF]/20' : 'bg-white/10'}`}>
            <audio ref={audioRef} src={src} preload="metadata" />

            <button
                onClick={togglePlay}
                className={`w-10 h-10 rounded-full flex items-center justify-center ${isOwn ? 'bg-[#00D4FF] text-black' : 'bg-white/20 text-white'
                    }`}
            >
                {isPlaying ? '⏸️' : '▶️'}
            </button>
            ...
            <div className="flex-1">
                <div className="w-full h-1.5 bg-white/20 rounded-full overflow-hidden">
                    <div
                        className={`h-full ${isOwn ? 'bg-[#00D4FF]' : 'bg-white/60'} transition-all`}
            ...
            <div className={`max-w-[70%] ${isOwn ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
                <div
                    className={`px-4 py-2.5 rounded-2xl ${isOwn
                            ? 'bg-gradient-to-r from-[#00D4FF] to-[#8B5CF6] text-black rounded-br-md'
