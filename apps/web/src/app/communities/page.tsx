...
function BulletinCard({ item }: { item: BulletinItem }) {
    ...
    return (
        ...
        >
            ...
            <h3 className="text-lg font-bold text-white mb-2 group-hover:text-[#7C8FFF] transition-colors">{item.title}</h3>
...
function CommunityCard({ community, index }: { community: Community; index: number }) {
    ...
    return (
        <motion.div
            className="bg-gradient-to-b from-white/[0.04] to-white/[0.01] border border-white/[0.06] rounded-2xl overflow-hidden hover:border-[#7C8FFF]/20 hover:shadow-[0_8px_32px_rgba(0,212,255,0.06)] transition-all duration-300 group relative h-64 cursor-pointer"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
        ...
        >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-5">
                <div className="flex items-center gap-2 mb-2">
                    {community.isPrivate ? (
                        ...
                        <span className="p-1 rounded bg-black/60 backdrop-blur text-[#7C8FFF]">{Icons.globe}</span>
                    )}
                    <span className="text-xs font-bold uppercase tracking-wider text-white/70">{community.role}</span>
                </div>
                <h3 className="text-xl font-bold text-white mb-1 group-hover:text-[#7C8FFF] transition-colors">{community.name}</h3>
            ...
            <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                <Link href={`/communities/${community.slug || community.id}`}>
                    <button className="bg-[#7C8FFF] text-white font-semibold text-sm px-5 py-2 rounded-xl hover:shadow-[0_0_16px_rgba(0,212,255,0.3)] transition-all duration-200">
                        Enter
                    </button>
                </Link>
            </div>
...
export default function CommunityHub() {
    ...
    return (
        <div className="min-h-screen bg-[#050508] text-white selection:bg-[#7C8FFF]/30">
            <NavigationSidebar />
                ...
                <div className="max-w-7xl mx-auto px-4 md:px-6 py-4 lg:py-8 space-y-6 lg:space-y-12">
                    ...
                    <div className="flex items-center justify-between gap-4 border-b border-white/10 pb-4 lg:pb-8">
                        ...
                        <Link href="/communities/create" className="flex-shrink-0">
                            <button className="bg-[#7C8FFF] text-white font-semibold text-xs lg:text-sm px-3 lg:px-5 py-2 rounded-xl hover:shadow-[0_0_16px_rgba(0,212,255,0.3)] transition-all duration-200 flex items-center gap-1.5 lg:gap-2">
                                {Icons.plus}
                    ...
                    <section>
                        <div className="flex items-center gap-2 mb-3 lg:mb-6">
                            <span className="text-[#7C8FFF]">{Icons.megaphone}</span>
                    ...
                    <section>
                        <div className="flex items-center justify-between mb-3 lg:mb-6">
                            <div className="flex items-center gap-2">
                                <span className="text-[#6070EE]">{Icons.users}</span>
                        ...
                        {isLoading ? (
                            ...
                            <div className="bg-gradient-to-b from-white/[0.04] to-white/[0.01] border border-white/[0.06] rounded-2xl overflow-hidden p-12 text-center">
                                ...
                                <Link href="/communities/create">
                                    <button className="text-[#7C8FFF] hover:underline font-medium">Create your first circle</button>
                                </Link>
                            </div>
                        ) : (
