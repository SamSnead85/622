            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }}
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-4 gap-4">
                    {tiers.map(tier => (
                        <button key={tier.tier} onClick={() => setSelectedTier(tier.tier)}
                            className={`p-5 rounded-2xl border text-left ${selectedTier === tier.tier ? 'bg-[#7C8FFF]/10 border-[#7C8FFF]/50' : 'bg-white/5 border-white/10'}`}>
                ...
                <div className="px-6 py-4 border-t border-white/10 flex justify-end">
                    <button onClick={handleSubscribe} disabled={selectedTier === currentTier || isProcessing}
                        className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-[#7C8FFF] to-[#6070EE] text-white font-medium disabled:opacity-50">
        ...
        <div className="p-6 rounded-2xl bg-white/5 border border-white/10">
            ...
            <div className="relative h-4 rounded-full bg-white/10 overflow-hidden mb-3">
                <motion.div initial={{ width: 0 }} animate={{ width: `${progress}%` }}
                    className={`absolute h-full rounded-full ${isComplete ? 'bg-green-500' : 'bg-[#7C8FFF]'}`} />
            </div>
            ...
            <button onClick={onDonate} disabled={isComplete}
                className="w-full py-3 rounded-xl bg-gradient-to-r from-[#7C8FFF] to-[#6070EE] text-white font-semibold disabled:opacity-50">
