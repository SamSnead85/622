        ...
        <>
            ...
            <aside className="hidden lg:flex fixed left-0 top-0 bottom-0 w-20 xl:w-64 bg-[#0A0A0F]/95 backdrop-blur-xl border-r border-white/5 flex-col p-4 z-40">
                    ...
                    {navItems.map((item) => (
                        <Link
                            ...
                            className={`relative flex items-center gap-3 px-3 py-3 rounded-xl transition-colors ${activeTab === item.id
                                ...
                                : (item as any).highlight
                                    ? 'bg-gradient-to-r from-[#7C8FFF]/20 to-[#6070EE]/20 text-[#7C8FFF] border border-[#7C8FFF]/30 hover:from-[#7C8FFF]/30 hover:to-[#6070EE]/30'
                        ...
                        >
                            {activeTab === item.id && (
                                <motion.div
                                    layoutId="activeTab"
                                    className="absolute inset-0 bg-[#7C8FFF]/10 border-l-2 border-[#7C8FFF] rounded-xl shadow-[inset_0_0_20px_rgba(0,212,255,0.1)]"
                                    transition={{
                                        type: 'spring',
                                        stiffness: 350,
                                        damping: 30,
                            ...
                            <span className="relative z-10">
                                ...
                                {item.hasNotification && (
                                    <span className="absolute -top-1 -right-1 w-2 h-2 bg-[#7C8FFF] rounded-full" />
                                )}
                            </span>
                            <span className="font-medium hidden xl:block relative z-10">{item.label}</span>
                            {(item as any).highlight && (
                                <span className="hidden xl:block ml-auto text-[10px] bg-[#7C8FFF]/20 px-2 py-0.5 rounded-full text-[#7C8FFF] relative z-10">NEW</span>
                            )}
                        </Link>
                    ))}
                </nav>
                ...
                <button
                    ...
                    className="flex items-center justify-center xl:justify-start gap-3 w-full px-4 py-3 rounded-xl bg-gradient-to-r from-[#7C8FFF] to-[#9A7A24] text-white font-semibold hover:shadow-[0_4px_20px_rgba(0,212,255,0.3)] transition-all duration-300 hover:scale-[1.02] mb-4"
                >
                    <PlusIcon size={18} />
                ...
                >
                    {user?.avatarUrl ? (
                        <img
                            ...
                            className="w-10 h-10 rounded-full object-cover ring-2 ring-[#7C8FFF]/20"
                        />
                    ) : (
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#7C8FFF] to-[#6070EE] flex items-center justify-center text-white font-bold ring-2 ring-[#7C8FFF]/20">
                    ...
                    {mobileNavItems.map((item) => (
                        <Link
                            ...
                            className={`relative flex flex-col items-center gap-0.5 min-w-[56px] min-h-[44px] justify-center active:scale-95 transition-transform ${
                                item.isCreate
                                    ? ''
                                    : activeTab === item.id ? 'text-[#7C8FFF]' : 'text-white/50'
                            }`}
                        >
                            {/* Create button gets a prominent style */}
                            {item.isCreate ? (
                                <div className="w-10 h-10 rounded-full bg-gradient-to-r from-[#7C8FFF] to-[#6070EE] flex items-center justify-center shadow-lg shadow-[#7C8FFF]/20 -mt-3">
                                ...
                                <>
                                    {activeTab === item.id && (
                                        <motion.div
                                            ...
                                            className="absolute inset-0 bg-[#7C8FFF]/10 rounded-xl"
                                            transition={{
                                                type: 'spring',
                                                stiffness: 350,
                                                damping: 30,
